...
//	NamedParameterJdbcTemplate jdbc;
	...
	public List<MovimientoEnCajaReporte> listarMovimientoEnCajaConFiltros(ReportArray reporte) {
		...
		+"FROM ("
		+"	SELECT cu.caja_id, cj.nombre AS caja, cu.usuario_id, CONCAT(us.apellidos,' ',  us.nombres) AS usuario, pd.id AS cod_pedido, pd.cliente_id, cl.nom_apell_rz AS cliente_proveedor, td.acronimo AS tipo_documento," 
		...
		+"	UNION all "
		+"	SELECT cu.caja_id, cj.nombre, cu.usuario_id,CONCAT(us.apellidos,' ',  us.nombres) AS usuario, 0,0, '','','',mc.create_at, ingreso_dinero, egreso_dinero, CASE  WHEN  tm.tipo = 'I' THEN 'Ingreso' WHEN tm.tipo= 'E' THEN 'Egreso' ELSE '' END, tm.movimiento, 'Caja' AS modulo "
